...
public class Controller {
        ...
        for (int i = 0; i < ocean.SIZE + 1; i++) {
            for (int j = 0; j < ocean.SIZE + 1; j++) {
                if (i == 0 && j == 0) {
                    ...
                    getGridBattleField().add(lbl, 0, j);
                } else if (j == 0) {
                    Label lbl = new Label(String.valueOf(i - 1));
                    GridPane.setHalignment(lbl, HPos.CENTER);
                    getGridBattleField().add(lbl, i, 0);
                } else {
                    getGridBattleField().add(createButton(""), j, i);
                }
            }
        }
        getTxtXCoord().requestFocus();
    ...
    private void doShootAtCoordinates(int row, int column) {
        ...
        ((Button) getGridBattleField().getChildren().get((row + 1) * ocean.SIZE + column + (row + 1))).fire();
    }
    ...
    private void reset() {
        ocean = new Ocean();
        ocean.placeAllShipsRandomly();
        for (Object obj : getGridBattleField().getChildren()) {
    ...
    private void markAreaAroundShip(Ship ship) {
        ...
        for (int i = Math.max(0, ship.getBowRow() - 1); i <= Math.min(ocean.SIZE - 1, ship.getBowRow() + addRow); i++) {
            for (int j = Math.max(0, ship.getBowColumn() - 1); j <= Math.min(ocean.SIZE - 1, ship.getBowColumn() + addColumn); j++) {
                ...
                Button currBtn = (Button) getGridBattleField().getChildren().get((i + 1) * ocean.SIZE + j + (i + 1));
    ...
    /**
     * @return Return the reference on the GridPane (battlefield).
     */
    private GridPane getGridBattleField() {
